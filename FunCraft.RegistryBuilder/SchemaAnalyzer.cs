...
{
    private static readonly HashSet<string> s_forceDouble = ["coordinate_scale"];

    private static readonly HashSet<string> s_forceInt =
    [
    ...
    private static readonly HashSet<string> s_forceString =
    [
        "title", "author",
    ];
    ...
    internal SchemaNode InferNode(JsonElement el, string fieldName) => el.ValueKind switch
    {
        ...
        JsonValueKind.Number when s_forceDouble.Contains(fieldName)
        ...
        JsonValueKind.Object when s_forceString.Contains(fieldName)
            => new ScalarSchemaNode(NbtTagType.String),
    ...
    private ScalarSchemaNode InferNumber(JsonElement el, string fieldName)
    {
        ...
        var iv = el.GetInt64();
        return new ScalarSchemaNode(s_forceInt.Contains(fieldName) || iv < -128 || iv > 127
            ? NbtTagType.Int
            : NbtTagType.Byte);
    }
    ...
    private static SchemaNode Merge(SchemaNode a, SchemaNode b, string fieldName)
    {
        ...
        Console.WriteLine($"  [dynamic] structural conflict on '{fieldName}': {a.GetType().Name} vs {b.GetType().Name}");
        return new DynamicSchemaNode();
    }
}